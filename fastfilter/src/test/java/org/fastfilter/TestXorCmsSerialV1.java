...
public class TestXorCmsSerialV1 {
    ...
    private static int size_neg;
           ...
           // for (TestFilterType type : TestFilterType.values()) {
                ...
                double[] mean_absolute_error_array = new double [size_test];
                    ...
                    for (int run_i = 1; run_i <= run; run_i += 1){
                        ...
                        int i = 0;
                        size_neg =100_000_000; // Number of keys not included in the Xor filter
        ...
        private static void test(int len_neg, int len2, int seedCMS, boolean log) {
            ...
            testFreq = 1_000_000; // Number of keys
            int seed_CMS = seedCMS;
            int len = len_neg;
            int dist = 1; // Distributions: for Pareto, dist = 0. For Gaussian, dist = 1.  For Poisson, dist = 2 
            ...
            Filter fCMS = XorCmsSerialV1.construct(uniqueKeys);
