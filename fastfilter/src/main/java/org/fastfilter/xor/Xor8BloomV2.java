 ...
 */
public class Xor8BloomV2 implements Filter {

    private static final int BITS_PER_FINGERPRINT = 7;
    private static final int HASHES = 3;
    //private static final int FACTOR_TIMES_100 = 123;
    private static final int FACTOR_TIMES_100 = 123;  // Modify to check the FPP degradation
    ...
    public static Xor8BloomV2 construct(long[] keys, long[] keysBloom) {
        return new Xor8BloomV2(keys, keysBloom);
    }

    public Xor8BloomV2(long[] keys, long[] keysBloom) {
                        ...
                        if (s==0){
                         ...
                         if (copykeys[l] != 0 ){
                            for (int hi = 0; hi < HASHES; hi++) {
                                ...
                                if (t2count[h] == 2) {
                                    //add(copykeys[l]);                                   
                                    for (int ji = 0; ji < HASHES; ji++) {
                                        int hh = getHash(copykeys[l], seed, ji);
                                        int newCount = --t2count[hh];
                                        t2[hh] ^= copykeys[l]; 
                                        if (newCount == 1) {
                                            alone[ji][alonePos[ji]++] = hh;
                                        }                                        
                                    }
                                    s=0;
                                    bl[bloomPos] = copykeys[l];
                                    copykeys[l] = 0;
                                    size2 --;
                                    bloomPos ++;
                                    break;
...
// Add keys on the Bloom Filter after complete the insertions on the Xor Filter
    ...
    public Xor8BloomV2(InputStream in) {
        try {
