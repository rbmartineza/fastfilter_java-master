 ...
 */
public class XorCmsSerialV1 implements Filter {
    ...
    private long seed;
    private long seedCMS;
    private int[] fingerprints;
    private int[] countCMS;
    private int[] fingerprintsCopy;
    private int[] CMS_Copy;
    ...
    public static XorCmsSerialV1 construct(long[] keys) {
        return new XorCmsSerialV1(keys);
    }

    public XorCmsSerialV1(long[] keys) {
        ...
        countCMS = new int[arrayLength];
        System.arraycopy(fp, 0, fingerprints, 0, fp.length);
        for(long x : keys) {
           for (int hi = 0; hi < HASHES; hi++) {
                int h = gethashCMS(x, seedCMS, hi);
                countCMS[h] += 1; 
           }
        }
        CMS_Copy = new int[arrayLength];
        System.arraycopy(countCMS, 0, CMS_Copy, 0, countCMS.length);
    ...
    public boolean mayContain(long key) {
        ...
        long hashc = Hash.hash64(key, seedCMS);
        ...
        int r0Cms = (int) hashc;
        int r1Cms = (int) Long.rotateLeft(hashc, 21);
        int r2Cms = (int) Long.rotateLeft(hashc, 42);
        int h0Cms = Hash.reduce(r0Cms, blockLength);
        int h1Cms = Hash.reduce(r1Cms, blockLength) + blockLength;
        int h2Cms = Hash.reduce(r2Cms, blockLength) + 2 * blockLength;
        ...
        if ((f & 0x000000ff) != 0)      
            ...
            int h0cCms =countCMS[h0Cms];
            int h1cCms =countCMS[h1Cms];
            int h2cCms =countCMS[h2Cms];
            
            if ( ( h0cCms + 1 ) >= 65000){
                System.out.println("Exceed the counter! counters c0= " +  (h0cCms + 1) );
                throw new AssertionError();
            }
            else if ( ( h1cCms + 1 ) >= 65000){
                System.out.println("Exceed the counter! counters c0= " +  (h1cCms + 1) );
                throw new AssertionError();
            }
            else if ( ( h2cCms + 1 ) >= 65000){
                System.out.println("Exceed the counter! counters c0= " +  (h2cCms + 1)  );
                throw new AssertionError();
            }
            countCMS[h0Cms] += 1;
            countCMS[h1Cms] += 1;
            countCMS[h2Cms] += 1;
            memAccess += 6;  // 3 memory access for find the 3 counters and 3 more to upgrade the counters (+1)
    ...
    private int gethashCMS(long key, long seed, int index) {
        long r = Long.rotateLeft(Hash.hash64(key, seed), 21* index);
        r = Hash.reduce((int) r, blockLength*3);
        return (int) r;
    }
    ...
    public void resetCms(long[] keys) {
        ...
        System.arraycopy(CMS_Copy, 0, countCMS, 0, CMS_Copy.length);
    }
...
@Override
    public int frequency(long key) {
            ...
            int h2Cms = Hash.reduce(r2Cms, blockLength) + 2 * blockLength;
            long lower;
            long c0 =countCMS[h0Cms];
            long c1 =countCMS[h1Cms];
            long c2 =countCMS[h2Cms];
